...
class Blinky(Enemy):
	...
	def update(self):
		self.short_path = self.shortest_path(self.grid_pos, self.player.get_grid_pos())
		for (x, y) in self.short_path:
		...
		if self.short_path[1][0] > self.grid_pos.x: # if next cell on path is to the right
			self.stored_direction = vec(1,0)
		elif self.short_path[1][0] < self.grid_pos.x: # if next cell on path is to the left
			self.stored_direction = vec(-1,0)
		elif self.short_path[1][1] > self.grid_pos.y: # if next cell on path is up
	...
	def shortest_path(self, enemy_pos, player_pos):
		# draw over old path
		for (x, y) in self.short_path:
