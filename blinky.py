...
class Blinky(Enemy):
	...
	def find_next_tile(self, enemy_pos, player_pos):
		...
		# Loop through possible directions to check if there is a wall or ghost house gate on that tile
		...
		for index, direction in enumerate(self.possible_directions):
			if self.app.map[int(self.grid_pos.y+direction[1])][int(self.grid_pos.x+direction[0])] != '1' and self.app.map[int(self.grid_pos.y+direction[1])][int(self.grid_pos.x+direction[0])] != 'G':
